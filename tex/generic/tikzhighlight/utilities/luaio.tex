...
% luaio@.
\directlua{ tex.enableprimitives('luaio@', {'luaescapestring'}) }
...
\def\lopenout#1#2{\directlua{ltio.open_handle(#1, "\luaio@luaescapestring{#2}")}}
\def\lcloseout#1{\directlua{ltio.close_handle(#1)}}
\def\lwrite#1#2{\directlua{ltio.write_handle(#1, "\luaio@luaescapestring{#2}")}}
